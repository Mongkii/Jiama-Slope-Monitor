<template>
  <div>
    <ve-bar :data="chart_data" :settings="chart_settings" :extend="chart_extend" :legend-visible="false" height="500px"
...
<script>
  ...
  const formatData = (data) => { // 将数据库内容转换为图表格式
    const l = Object.keys(data)
      .filter(key => key.startsWith('p')).length + 1; // 获取点的数量
    let chart_data_rows = [];
    for (let i = 1; i < l; i++) {
      chart_data_rows.push({
        'point': `#${i}`,
        'offset': data[`p${i}`]
      });
    }
  ...
  export default {
    ...
    computed: {
      chart_data() {
        ...
        const latest_data = database.get('1').getLatest();
        return {
          columns: ['point', 'offset'],
          rows: formatData(latest_data)
        };
      }
    },
    props: {
